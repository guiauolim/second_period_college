...
public class VisaoProduto extends JFrame {
	...
	private JComboBox<String> cboDescricao = new JComboBox<String>();
	...
	// DETALHAMENTO
		...
		for (Tipo objTipo : Tipo.recuperarTodos()) {
			cboDescricao.addItem(objTipo.getDescricao());
		}
	...
	// BOTÕES	
		btnGravar.setBounds(450, 500, 100, 30);
		 ...
		 btnGravar.addActionListener(new ControladorGravar(txtNome, txtPreco, cboDescricao, chkPerecivel, txtAreaDetalhamento));
		 btnLimpar.setBounds(550, 500, 100, 30);
		 objPainel.add(btnLimpar);
		 btnLimpar.addActionListener(new ControladorLimpar(txtNome, txtPreco, cboDescricao, chkPerecivel, txtAreaDetalhamento));
